...
void CloseFingers:: closeFingers()
{
	int result;
...
/*			pointToSend.Position.Type = ANGULAR_VELOCITY;
			...
			pointToSend.Position.Fingers.Finger1 = -20;
			pointToSend.Position.Fingers.Finger2 = 20;
			pointToSend.Position.Fingers.Finger3 = 20;

			for(int i = 0; i < 300; i++)
			{
				//We send the velocity vector every 5 ms as long as we want the robot to move along that vector.
				cout<<endl<<"sending velocity";
			...
			for(int i = 0; i < 300; i++)
			{
                               cout<<endl<<"sending actuator velocity";
				//We send the velocity vector every 5 ms as long as we want the robot to move along that vector.
				MySendBasicTrajectory(pointToSend);
				usleep(5000);
			}*/
...
//			MyMoveHome();
			...
			pointToSend.Position.Actuators.Actuator6 = currentCommand.Actuators.Actuator6;
                        pointToSend.Position.Fingers.Finger1 =4500;
                        pointToSend.Position.Fingers.Finger2 =4500;
                        pointToSend.Position.Fingers.Finger3 =4500;
			cout << "*********************************" << endl;
